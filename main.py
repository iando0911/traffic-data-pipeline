"""
台灣交通事故大數據分析管線 v2.7.0

修正項目：
  1. [v2.7] 移除 sync_to_s3() 與 import boto3
            S3 部署職責移交 GitHub Actions deploy-aws job（aws s3 sync），
            ETL 本體僅負責資料處理與靜態檔案產出，符合單一職責原則
  2. 修正 __main__ 執行順序，確保 run_pipeline() 產出後才執行 sync_to_s3()
  3. 刪除 fetch_latest_accident_urls 中重複損壞的邏輯，保留完整的 12 個靜態網址 Fallback
  4. 全面替換 requests/cloudscraper 為 curl_cffi，完美繞過政府 WAF 阻擋
  5. 完整保留原作者的 Plotly 排版、註解與所有視覺化邏輯
  6. [v2.6] 新增「本地資料夾匯入」：自動讀取 data/ 目錄下的 ZIP/CSV，突破 WAF 終極方案
  7. [v2.6] 新增「智慧去重機制」：避免本地完整檔案與線上舊版檔案重複計算
  8. [v2.6.1] 修正 _collect_urls：改以值內容辨識下載連結，不再依賴 key 名稱，
...
def make_session():
    """建立帶有真實瀏覽器指紋的 Session，用來繞過政府 WAF"""
...
def fetch_latest_accident_urls() -> list[str]:
    ...
    try:
        for ds_id in [dataset_ids["A1"], dataset_ids["A2"]]:
            resp = session.get(
                f"{api_endpoints[0]}{ds_id}", headers=HEADERS, timeout=10
            )
    ...
    if not dynamic_urls:
        ...
        try:
            for ds_uuid in [dataset_ids["A1_alt"], dataset_ids["A2_alt"]]:
                resp = session.get(
                    f"{api_endpoints[1]}{ds_uuid}", headers=HEADERS, timeout=10
                )
                if resp.status_code == 200:
    ...
    if not unique_urls:
        print(
            "      ⚠️ 所有動態爬蟲路由皆失效 "
            "(可能遭政府 WAF 阻擋 GitHub IP)，啟用策略 3：載入靜態歷史連結庫"
        )
        unique_urls = [
            # A1
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "02D40248-7CAA-4354-82EA-E27AB8DCAB39/resource/"
            "F0367893-0E0D-4E5A-A6BC-430AFAD27E83/download",
            # A2
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "986931B3-0E46-4F94-BF52-A2911499301F/resource/"
            "7C775EF1-A689-451D-AD02-1265F7D41ADC/download",
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "986931B3-0E46-4F94-BF52-A2911499301F/resource/"
            "6A63F59F-2D81-45E0-A59E-253DB0609DFF/download",
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "986931B3-0E46-4F94-BF52-A2911499301F/resource/"
            "406D4A22-E25A-4C40-91EC-5343B27ADEBA/download",
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "986931B3-0E46-4F94-BF52-A2911499301F/resource/"
            "3DC7F6AA-438C-4838-8BB5-62C953711445/download",
            "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/"
            "986931B3-0E46-4F94-BF52-A2911499301F/resource/"
            "9439E46D-1536-4073-8523-92024E9FF8BE/download",
        ]
...
def check_monthly_completeness(
    monthly_df: pd.DataFrame, threshold: float = 0.2
) -> list[int]:
    monthly_total = (
...
def run_pipeline():
                ...
                try:
                    with zipfile.ZipFile(file_path) as z:
                        ...
                        for fname in csv_files:
                            ...
                            if "schema" in fname_lower or fname_lower in [
                                "file.csv", "manifest.csv"
                            ]:
                                continue
                            df = safe_read_csv(
                                z.read(fname),
                                label=f"{file_path.name} -> {fname}",
                            )
                            if df is not None:
                                dfs.append(df)
    ...
    if not dfs:
        print(
            "\n⚠️ 警告：無法取得任何資料（線上 + 本地 + 快取均失敗），"
            "請檢查網路或 API 端點。"
        )
        raise SystemExit(1)

    if download_success_count < len(CONFIG["accident_urls"]):
        failed = len(CONFIG["accident_urls"]) - download_success_count
        print(f"\n⚠️  注意：{failed} 個線上來源失敗，資料可能非最新版本")
    ...
    if dedup_cols:
        ...
        if before_drop > len(df_clean):
            print(
                f"   ⚠️ 去重啟動：成功移除 "
                f"{before_drop - len(df_clean):,} 筆因新舊檔案重疊的重複資料"
            )

    n_first_party = len(df_clean)
    print(
        f"   第一當事者純化後：{n_first_party:,} 筆"
        f"（{n_first_party / n_raw * 100:.1f}% of raw）"
    )
    ...
    date_col = next(
        (c for c in ["發生日期", "事故發生日期"] if c in df_clean.columns), None
    )
    time_col = next(
        (c for c in ["發生時間", "事故發生時間"] if c in df_clean.columns), None
    )
    ...
    age_gender_missing_rate = (
        (age_invalid_mask | gender_invalid_mask).sum() / max(n_first_party, 1) * 100
    )
    ...
    if len(df_clean) > 0:
        ...
        if incomplete_months:
            print(
                f"   ⚠️  月份資料不完整（件數異常低）：{incomplete_months} 月，"
                "圖表將標示警示虛線"
            )
        ...
        fig_age_compare.update_layout(
            title=(
                f"📋 年齡段事故率對照表（絕對件數 vs 暴露率校正）{SNAPSHOT_NOTE}"
            ),
            height=350,
        )
    ...
    if stats_summary:
        ...
        dashboard_data = {
            ...
            "age_rate_adjusted": (
                ...
                if not age_rate_df.empty else []
            ),
        }

        with open(
            CONFIG["output_dir"] / "dashboard_data.json", "w", encoding="utf-8"
        ) as f:
    ...
    if web_dir.exists():
        ...
        print(
            "   ⚠️  web/ 目錄不存在，略過靜態文件複製"
            "（部署後網站將缺少前端頁面）"
        )

    print("\n" + "=" * 60)
    print("🚀 管線 v2.7.0 執行完畢！")
    print(f"   輸出目錄：{CONFIG['output_dir'].resolve()}")
    print(f"   Git SHA：{GIT_SHA}")
    if incomplete_months:
        print(f"   ⚠️  請注意：{incomplete_months} 月份資料可能不完整")
    print("=" * 60)
    # S3 sync 由 GitHub Actions deploy-aws job 的 aws s3 sync 負責
    # main.py 職責：下載 → 清洗 → 視覺化 → 輸出靜態檔案至 output/
